...
regional_disk=$(ls /dev/disk/by-id | grep google-${device_name_region})
zonal_disk=$(ls /dev/disk/by-id | grep google-${device_name_zonal})

if [[ $regional_disk == "google-${device_name_region}" && $zonal_disk == "google-${device_name_zonal}" ]]; then
